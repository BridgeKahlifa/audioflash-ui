...
export type ApiCreateFlashcard = components["schemas"]["CreateFlashcardRequest"];
...
export async function fetchProfile(token: string): Promise<ApiProfile> {
  const res = await fetch(`${API_BASE_URL}/profile`, { headers: authHeaders(token) });
  return parseJson<ApiProfile>(res);
}

export async function updateProfile(token: string, body: ApiUpdateProfile): Promise<ApiProfile> {
...
export async function fetchSessions(token: string): Promise<ApiSession[]> {
  const res = await fetch(`${API_BASE_URL}/sessions`, { headers: authHeaders(token) });
  return parseJson<ApiSession[]>(res);
}

export async function fetchSessionStats(token: string): Promise<ApiSessionStats> {
  const res = await fetch(`${API_BASE_URL}/sessions/stats`, { headers: authHeaders(token) });
  return parseJson<ApiSessionStats>(res);
}

export async function createSession(token: string, body: ApiCreateSession): Promise<ApiSession> {
...
export async function bulkCreateFlashcards(cards: ApiCreateFlashcard[]): Promise<ApiLessonCard[]> {
