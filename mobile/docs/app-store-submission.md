...
SIGN-IN FOR REVIEW
------------------
The normal sign-in options are Google OAuth and a one-time code sent by email,
neither of which is convenient for review. We have added a dedicated password
...
---------------------------------
Google Sign-In is offered alongside two equivalent privacy-preserving options
that do not involve a third-party social provider: a one-time code sent to the
user's email address, and a passkey. Both collect only the user's email address,
neither shares data with a social network, and neither is used for advertising
or tracking.
